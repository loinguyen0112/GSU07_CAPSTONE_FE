...
CLASS lhc_Request DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    METHODS validateDepartment FOR VALIDATE ON SAVE
      IMPORTING keys FOR Request~validateDepartment.
...
*  ENDMETHOD.
  ...
  METHOD validateEmail.
    READ ENTITIES OF zi_iam_lreq_hdr IN LOCAL MODE
      ENTITY Request FIELDS ( ReqType Email ) WITH CORRESPONDING #( keys )
      RESULT DATA(lt_requests).
  ...
  METHOD validateDepartment.
    READ ENTITIES OF zi_iam_lreq_hdr IN LOCAL MODE
      ENTITY Request FIELDS ( ReqType Department ) WITH CORRESPONDING #( keys )
      RESULT DATA(lt_requests).

    LOOP AT lt_requests INTO DATA(ls_request).
      IF ls_request-ReqType <> 'J' AND ls_request-ReqType <> 'M'.
        CONTINUE.
      ENDIF.
      ...
      IF ls_request-Department IS INITIAL.
        CONTINUE.
      ENDIF.

      SELECT SINGLE DepartmentID
        FROM zi_iam_dept_vh
        WHERE DepartmentID = @ls_request-Department
  ...
  METHOD validateJoiner.
    READ ENTITIES OF zi_iam_lreq_hdr IN LOCAL MODE
      ENTITY Request FIELDS ( ReqUuid ReqType TargetUser ) WITH CORRESPONDING #( keys )
      RESULT DATA(lt_requests).

    LOOP AT lt_requests INTO DATA(ls_request).
      IF ls_request-ReqType <> 'J' OR ls_request-TargetUser IS INITIAL.
        CONTINUE.
      ENDIF.

      SELECT SINGLE bname
        FROM usr02
        WHERE bname = @ls_request-TargetUser
      ...
      SELECT SINGLE req_id
        FROM ziam_lreq_hdr2
        WHERE req_type = 'J'
          AND target_user = @ls_request-TargetUser
          AND status <> '04'
          AND req_uuid <> @ls_request-ReqUuid
        INTO @DATA(lv_duplicate_req_id).
      IF sy-subrc = 0.
        APPEND VALUE #( %tky = ls_request-%tky ) TO failed-request.
        APPEND VALUE #( %tky = ls_request-%tky
                        %msg = new_message(
...
*  METHOD validateUser.
*    READ ENTITIES OF zi_iam_lreq_hdr IN LOCAL MODE
*      ENTITY Request FIELDS ( TargetUser ReqType ) WITH CORRESPONDING #( keys )
*      RESULT DATA(lt_requests).
*
*    LOOP AT lt_requests INTO DATA(ls_request).
*      " Chỉ bắt lỗi trùng User nếu đây là Joiner (Tạo mới)
*      IF ls_request-ReqType = 'J'.
*        SELECT SINGLE bname FROM usr02 WHERE bname = @ls_request-TargetUser INTO @DATA(lv_bname).
*        IF sy-subrc = 0.
*          APPEND VALUE #( %tky = ls_request-%tky ) TO failed-request.
*          APPEND VALUE #( %tky = ls_request-%tky
